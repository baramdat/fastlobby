...
class BuildingAdminController extends Controller
{
    ...
    public function userCount(Request $request)
    {
        ...
        $result = User::where('parent_id',Auth::user()->id);
        // $result = $result
        if ($filterTitle != '') {
    ...
    public function users(Request $request)
    {
        ...
        $result = $result->where('parent_id', $user->id);
        }
        if ($filterTitle != '') {
        ...
        if (isset($user) && sizeof($user) > 0) {
            ...
            foreach ($user as $usr) {
                ...
                if (Auth::user()->id == $usr->id || (Auth::user()->hasRole('Tenant') && $usr->hasRole('Admin'))) {
                    ...
                    $btn = '
                        <li>
                <a href="/integrator/user/detail/' . $usr->id . '" class="bg-primary text-white border-primary border text-center"
                            style="height:30px !important; width:30px !important; line-height:30px !important;">
                            <i class="fe fe-eye "> </i> </a>
                    </li>
                    <li>
                <a href="/integrator/user/edit/' . $usr->id . '" class="btnEdit bg-success text-white border-success border"
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-3 text-end">
                                        <div class="ms-auto mt-1 file-dropdown">
                                            ...
                                            <a class="dropdown-item btnEdit" href="/integrator/user/edit/' . $usr->id . '"><i class="fe fe-edit me-2"></i> Edit</a>  
                                            ' . $btun . '               
                                            <a class="dropdown-item" href="/integrator/user/detail/' . $usr->id . '" ><i class="fe fe-eye me-2"></i>
                                            Detail</a>
                                            </div>
                                        </div>
                                    </div> 
    ...
    {
        return view('templates.Integrator.building_admin.add');
    ...
    public function editUser($id)
    {
        ...
        if ($user) {
            return view('templates/Integrator/building_admin/edit', ['user' => $user]);
        } else {
            return view('templates.404');
        }
    }
    ...
    public function userDetail($id)
    {
        $user = User::find($id);
        if (!empty($user)) {
            return view('templates.Integrator.building_admin.detail', ['user' => $user]);
        } else {
