...
class userController extends Controller
{
    ...
    public function add(Request $request)
    {
        ...
        public function addQrCodeToUser()
    {
        $users = User::all();
    ...
    public function userCountForChat(Request $request)
    {
        $filterRole = $request->filterRole;
        $filterTitle = $request->filterTitle;
        $filterLength = $request->filterLength;
    ...
    public function usersForChat(Request $request)
    {
        $filterRole = $request->filterRole;
        $filterTitle = $request->filterTitle;
        $filterLength = $request->filterLength;
        $result = User::query()->where('id', '!=', Auth::user()->id);
        if ($filterTitle != '') {
        ...
        $user = $result->take($filterLength)->skip($request->offset)->get();
    ...
    public function users(Request $request)
    {
        ...
        // }
        $user = $result->take($filterLength)->skip($request->offset)->where('id', '!=', Auth::user()->id)->orderBy('id', 'DESC')->get();
        if (isset($user) && sizeof($user) > 0) {
            $html = '';
            foreach ($user as $usr) {
                if ($usr->image == NULL) {
